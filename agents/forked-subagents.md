# Forked / isolated subagents (required)
...
- Prefer **forked / isolated** subagent or **Task** runs so the child does **not** merge its full transcript into the parent by default (use the product’s “forked subagents” / separate task flow when available).
